# -*- coding: utf-8 -*-
"""
模型: Barot Outer Approximation (K-R预处理)
"""
...
def algo(data):
    ...
    # 2. 应用K-R预处理并聚合
    # 注意: K-R需要更复杂的实现，这里简化为与标准预处理相同
    L = tools.preconditioning_matrix(A_list[0], b_list[0])
    A_approx = L @ A_list[0]
    b_approx = np.sum([L @ b for b in b_list], axis=0)
