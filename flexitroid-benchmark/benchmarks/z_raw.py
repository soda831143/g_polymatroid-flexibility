...
def algo(data):
    ...
    for obj in data["objectives"]:
        if obj == "cost":
            ...
            x = model.addMVar(
                shape=T, lb=-gp.GRB.INFINITY, vtype=gp.GRB.CONTINUOUS, name="x"
            )
            ...
            model.addConstr(A_approx @ x <= b_approx)
            model.addConstrs(
                (-m <= demand_aggr[t] + x[t] for t in range(T)), name="left"
            )
            model.addConstrs(
                (demand_aggr[t] + x[t] <= m for t in range(T)), name="right"
            )
            model.optimize()
            algo_res[f"{obj}_time"] = time.time() - t0
            if model.status != 2:  # 2 is optimal
